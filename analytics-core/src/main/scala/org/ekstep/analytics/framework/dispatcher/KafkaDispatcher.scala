...
object KafkaDispatcher extends IDispatcher {
    ...
    def dispatch(events: Array[String], config: Map[String, AnyRef])(implicit fc: FrameworkContext): Array[String] = {
        ...
        val compression = config.getOrElse("compression", "snappy").asInstanceOf[String];
        if (null == brokerList) {
        ...
        KafkaEventProducer.sendEvents(events, topic, brokerList, batchSize, lingerMs, compression)
        events
    }
    ...
    def dispatch(config: Map[String, AnyRef], events: RDD[String])(implicit sc: SparkContext, fc: FrameworkContext) = {
        ...
        events.foreachPartition((partitions: Iterator[String]) => {
            val kafkaSink = KafkaSink(_getKafkaProducerConfig(brokerList, batchSize, lingerMs, compression));
    ...
    private def _getKafkaProducerConfig(brokerList: String, batchSize: Integer, lingerMs: Integer, compression: String): HashMap[String, Object] = {
