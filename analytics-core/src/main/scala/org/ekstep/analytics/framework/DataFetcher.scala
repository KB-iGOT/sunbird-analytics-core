...
import org.ekstep.analytics.framework.fetcher.{AzureDataFetcher, DruidDataFetcher, S3DataFetcher, S3LikeDataFetcher}
    ...
    def fetchBatchData[T](search: Fetcher)(implicit mf: Manifest[T], sc: SparkContext, fc: FrameworkContext): RDD[T] = {
        ...
        val keys = search.`type`.toLowerCase() match {
            case "s3-like" =>
                ...
                S3LikeDataFetcher.getObjectKeys(search.queries.get);
            case "s3" =>
